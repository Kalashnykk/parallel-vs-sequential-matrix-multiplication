...
// Should we run the parallel algorithm?  This allows disabling the MPI portion
bool testParallel = true;

// Print matrix function declaration
void printMatrix(int matrix[N][N]);
void multiplyMatrixChunk(int M, int matrix1[M][N], int matrix2[N][N], int productMatrix[M][N], Algorithm algo);
void multiplyMatrix(int matrix1[N][N], int matrix2[N][N], int productMatrix[N][N], Algorithm algo);
...
int main(int argc, char **argv)
{
    ...
    /* sanity check for mutually exclusive options */
    ...
    if (processorRank == 0)
    {
        ...
        if (testParallel) {
            printf("\nMultiplication of %dx%d int matrices using %d processor(s).\n", N, N, numberOfProcessors);
            ...
            for (int algo = 0; algo < ALGO_COUNT; algo++) {
                ...
                // Iterate through all of the workers and assign work for the current algorithm
    ...
    {
        if (!testParallel) {
            /* parallel work disabled; exit immediately on workers */
            MPI_Finalize();
            return 0;
        }
